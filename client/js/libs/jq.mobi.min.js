...
this.bind(a,callback):this.trigger(a)}});return e}(window);"$"in window||(window.$=jq);if(!window.numOnly)window.numOnly=function(f){if(f===void 0||f==="")return 0;if(isNaN(parseFloat(f)))if(f.replace)f=f.replace(/[^0-9.-]/,"");else return 0;return parseFloat(f)}};
